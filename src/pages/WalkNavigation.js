...
function WalkNavigation() {
  ...
  async function setSign(i) {
    ...
    blob = null;
    if (i == 0) {
      await testFun(`${features[i].properties.description}하세요.`);
      setLat(features[2].geometry.coordinates[1]);
      setLong(features[2].geometry.coordinates[0]);
    }
    document.getElementById(
      "context"
    ).innerText = `${features[i].properties.description}하세요.`;
      ...
      if (distance <= 0.005) {
        ...
        if (features[i + 2].properties.facilityType == "15") {
          ...
          document.getElementById("context").innerText = `${
            features[i + 2].properties.description
          }하세요.`;
          blob = await testFun(
            `${features[i + 2].properties.description}하세요.`
          );
