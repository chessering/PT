...
function Report() {
  ...
  const [doubleTouch, setDoubleTouch] = useState(true);


  const ChangeTouch = () => {
    TTS.testFun("카메라로 이동하는 버튼입니다.");
    console.log(doubleTouch);
    setDoubleTouch(doubleTouch === true ? false : true);
    console.log(doubleTouch);
  ...
  const handleImageSelect = async (event) => {
    ...
    if (file) {
      ...
      console.log(body);
      await axios.post('https://safe-roadmap.shop/photo/analysis?x=127.254424&y=37.24141414',body,
        {
          headers : {
        ...
        .then((response)=>{
          ...
          displayName = response.data.result.predictResult.displayName;
    ...
    console.log(displayName, photoId);
  };
  return (
    <div className="Report">
      <div className="Buttons">
        <button className="Camera" onClick={ChangeTouch}
            >
          <label htmlFor="file">
            <svg
